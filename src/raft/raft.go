...
func (rf *Raft) Index(idx int) int {
	return idx - rf.log.Entries[0].Index
}
...
func (rf *Raft) CondInstallSnapshot(lastIncludedTerm int, lastIncludedIndex int, snapshot []byte) bool {
	...
	if rf.log.lastLog().Index < lastIncludedIndex {
		//若快照的最后一个log比当前最新的log还晚,那么清空log中除了0位的log
		rf.log.Entries = rf.log.Entries[:1]
	} else {
		//清除log中[1,realIndex]之间的数据
		realIndex := rf.Index(lastIncludedIndex)
		rf.log.Entries = append(rf.log.Entries[:1], rf.log.Entries[realIndex+1:]...)
	}
	//3.保存快照数据
	rf.snapshot = snapshot
	//4.更改日志占位节点
	rf.log.Entries[0].Index = lastIncludedIndex
...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	//1.获取需要压缩末尾日志的数组内索引
	realIndex := rf.Index(index)
	lastLogEntry := rf.log.Entries[realIndex]
	...
	rf.snapshot = snapshot
	//4.更改日志占位节点
	rf.log.Entries[0].Index = lastLogEntry.Index
	rf.log.Entries[0].Term = lastLogEntry.Term
	//5.持久化
	rf.persistStateAndSnapshot()
