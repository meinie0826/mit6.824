...
func (rf *Raft) appendEntries(heartbeat bool) {
	for peer, _ := range rf.peers {
		...
		go func(server int) {
			rf.mu.Lock()
			// rules for leader 3
			lastLog := rf.log.Entries[len(rf.log.Entries)-1]

			if lastLog.Index >= rf.nextIndex[server] || heartbeat {

				if rf.nextIndex[server] <= rf.log.Entries[0].Index {
				...
				args := AppendEntriesArgs{
					...
					PrevLogTerm:  rf.log.at(rf.nextIndex[server] - 1).Term,
					//Entries:      rf.logEntries[rf.binaryFindRealIndexInArrayByIndex(rf.nextIndex[server]):],
					LeaderCommit: rf.commitIndex,
				}
				args.Entries = make([]Entry, rf.log.len()-args.PrevLogIndex)
				copy(args.Entries, rf.log.slice(args.PrevLogIndex+1))
				rf.mu.Unlock()
				rf.leaderSendEntries(server, &args)
				rf.mu.Lock()
			}
			rf.mu.Unlock()
		}(peer)
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.PrevLogIndex < rf.log.Entries[0].Index {
		reply.XTerm = -1
		reply.Term = 0
		reply.XLen = rf.log.Entries[0].Index
		reply.Success = false
		return
	}
